...
function Afficher_commande() {
  ...
  useEffect(() => {
    ...
    axios.post('http://localhost:8081/list_com')
      .then((response) => {
  ...
  const handleDelete = async (idcult) => {
    try {
      await axios.delete('http://localhost:8081/list_com/' + idcult)
  ...
  return (
    <div>
      <header>
        ...
        <Link to={'/ajout_com'} className="btn btn-success ms-2">
            ...
            <tbody>
              {data.map((item) => (
                <tr key={item.idcom}>
                  ...
                  <td>{item.prod}</td>
                  <td>{item.qt}</td>
                  <td>{formatDateTime(item.daty)}</td>
              
     

                  <td >

                    <Link to={`../edit_com/${item.idcom}`} className='btn  ms-2'><i className='bi bi-pencil-square text-success fs-2' ></i></Link>
                    <button
                      className='btn ms-4'
                      onClick={async () => {
                        ...
                        if (result.isConfirmed) {
                          try {
                            await handleDelete(item.idcom);
                            // Vous pouvez également ajouter une SweetAlert2 pour indiquer que la suppression a réussi
                            Swal.fire('Supprimé !', 'Le commande a été supprimé avec succès.', 'success');
                          } catch (err) {
                            // En cas d'erreur, affichez une autre alerte SweetAlert2
                            Swal.fire('Erreur', 'Une erreur est survenue lors de la suppression du commande.', 'error');
                          }
                        }
                      }}
                    >
