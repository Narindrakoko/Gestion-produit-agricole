...
import legume from './sary/vomaina.jpg'
import legume2 from './sary/vomaina1.jpg'
import petit from './sary/vomaina2.jpg'
import vomaina from './sary/vomaina3.jpg'
import tsaramaso from './sary/petitepoid.jpg'
import haricotvert from './sary/vomaina5.jpg'
...
const generateInvoicePDF = (userData, selectedProducts) => {
  ...
  let invoiceContent = `Voici votre facture\n`;
...
useEffect(() => {
    const fetchData = async () => {
        try {
            const response = await axios.get("http://localhost:8081/produit02");
...
<IconButton aria-label="cart" onClick={handleSubmit} >
      <StyledBadge badgeContent={numberOfLines} color="secondary">
        <ShoppingCartIcon style={{ fontSize:'6vh', color:'black' }}/>
                  ...
                  <div class="card mb-3 bg-transparent" style={{ borderStyle: 'none' }}>
                    <div style={{ marginBottom: '2%' }}>

                      <Nav fill variant="tabs" defaultActiveKey="/content">
                        <Nav.Item className='ct1' >

                          <Link to="/content" className="p-1 fs-4 " style={{ color: '#2C2D30', borderRadius: '3px', textDecoration: 'none' }}>
                            <span className="fs-5" >Legumes</span>
                          </Link>
                        </Nav.Item>
                        <Nav.Item className='ct1'>
                          <Link to="/content2" className="p-1 fs-4 " style={{ color: '#2C2D30', textDecoration: 'none', padding: '10%', transition: 'color 0.3s' }} hover={{ color: 'black', backgroundColor: 'initial' }}>
                            <span className="fs-5" >Fruits</span>
                          </Link>
                        </Nav.Item>
                        <Nav.Item className='ct1'>
                          <Link to="/content3" className="p-1 fs-4" style={{ color: 'black', background: '#217e38', textDecoration: 'none', padding: '10%', transition: 'color 0.3s' }} hover={{ color: 'black', backgroundColor: 'initial' }}>
                            <span className="fs-5" >Légumineuses</span>
                          </Link>
                        </Nav.Item>
                        <Nav.Item className='ct1'>
                          <Link to="/afficherautre" className="p-1 fs-4 " style={{ color: '#2C2D30', textDecoration: 'none', padding: '10%', transition: 'color 0.3s' }} hover={{ color: 'black', backgroundColor: 'initial' }}>
        ...
        {produits.map((produit) => {
            if (produit.prod === 'Haricot') {
                return (
                  <Card  style={{ width: '15rem', marginInline: '4%', marginTop: '2%' }}>
                     <StyledBadge1 badgeContent={produit.qt} color="secondary">
                  </StyledBadge1>
                  <img src={legume} alt="Haricot" />
                 
                  <Card.Body>
                    <Card.Title style={{ fontSize: '1.2rem' }}>{produit.prod}</Card.Title>
                    <Card.Text>
                      {produit.prix} ariary/KG 
                    </Card.Text>
                    <Button style={{ background: 'transparent', backgroundColor: 'transparent', color: 'black', borderBlockColor: 'green' }}
                     onClick={() => handleAddToCart({ idprod: produit.idprod, prod: produit.prod, descri: produit.descri, prix: produit.prix})}>
                      <span>Acheter </span><i className='bi bi-cart-plus text-success fs-6'></i>
                    </Button>
                  </Card.Body>
                </Card>
                );
            } else if (produit.prod === 'Maïs') {
                return (
                  <Card  style={{ width: '15rem', marginInline: '4%', marginTop: '2%' }}>
                     <StyledBadge1 badgeContent={produit.qt} color="secondary">
                  </StyledBadge1>
                  <img src={haricotvert} alt="Maïs" />
                  <Card.Body>
                    <Card.Title style={{ fontSize: '1.2rem' }}>{produit.prod}</Card.Title>
                    <Card.Text>
                      {produit.prix} ariary/KG
            ...
            } else if (produit.prod === 'Lentille') {
              return (
                <Card  style={{ width: '15rem', marginInline: '4%', marginTop: '2%' }}>
                   <StyledBadge1 badgeContent={produit.qt} color="secondary">
                  </StyledBadge1>
                <img src={petit} alt="Lentille" />
                <Card.Body>
                  <Card.Title style={{ fontSize: '1.2rem' }}>{produit.prod}</Card.Title>
                  <Card.Text>
                    {produit.prix} ariary/KG
          ...
          } else if (produit.prod === 'Arachide') {
            return (
              <Card  style={{ width: '15rem', marginInline: '4%', marginTop: '2%' }}>
                 <StyledBadge1 badgeContent={produit.qt} color="secondary">
                  </StyledBadge1>
              <img src={vomaina} alt="Arachide" />
              <Card.Body>
                <Card.Title style={{ fontSize: '1.2rem' }}>{produit.prod}</Card.Title>
                <Card.Text>
                  {produit.prix} ariary/KG
        ...
        } else if (produit.prod === 'Petit poid') {
          return (
            <Card  style={{ width: '15rem', marginInline: '4%', marginTop: '2%' }}>
               <StyledBadge1 badgeContent={produit.qt} color="secondary">
                  </StyledBadge1>
            <img src={tsaramaso} alt="Petit poid" />
            <Card.Body>
              <Card.Title style={{ fontSize: '1.2rem' }}>{produit.prod}</Card.Title>
              <Card.Text>
                {produit.prix} ariary/KG
      ...
      } else if (produit.prod === 'Haricot Adzusi') {
        return (
          <Card  style={{ width: '15rem', marginInline: '4%', marginTop: '2%' }}>
             <StyledBadge1 badgeContent={produit.qt} color="secondary">
                  </StyledBadge1>
          <img src={legume2} alt="Haricot Adzusi" />
          <Card.Body>
            <Card.Title style={{ fontSize: '1.2rem' }}>{produit.prod}</Card.Title>
            <Card.Text>
              {produit.prix} ariary/KG
